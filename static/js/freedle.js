...
var compliments = ["Black Magic", "Lucky", "Impressive", "Well Done", "Great", "Phew"];
...
function evaluate(guess, guess_no) {
  ...
  if (!(all_answers.includes(guess_string)) && !(valid_words.includes(guess_string))) {
    ...
    row.on('animationend', function(){$(this).removeClass('invalid')});
...
function gameLost() {
  ...
  showToaster('The Word Was: ' + getSolution(today, start_date, all_answers));
...
function showToaster(message) {
  ...
  toast.text(message);
  toaster.removeClass('hidden').css({'top': '-50%'})
  .animate(
    { top:'10%' },
    {
      duration: 1000,
